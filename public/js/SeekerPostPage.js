...
$.get(`/api/v1/seeker/${USER_ID}`, async (seekerData) => {
    ...
    $("#button-apply, #button-apply-mobile").click(function(){
    ...
    $.get(`/api/v1/posts/${URL_ID}`, async (postData) => {
        ...
        if(postData.datas.post_resume_req) {
            $("#required-resume").removeClass("hidden")
        }
        if(postData.datas.post_portfolio_req) {
            $("#required-portfolio").removeClass("hidden")
            $("#popup-required-portfolio").on("input", function(){
                if(!this.value) $("#button-submit-apply").prop("disabled", true)
                else $("#button-submit-apply").prop("disabled", false)
            })
        }

        const APPLIED_ID = seekerData.applied.map(apply => apply.id)
        if(APPLIED_ID.includes(postData.datas.id)){
            $("#button-apply, #button-apply-mobile").prop("disabled", true)
            $("#seeker-applied, #seeker-applied-mobile").removeClass("hidden")
        ...
        if(RECRUITER.rec_verified) $("#is-verified").removeClass("hidden")
        $("#number-of-position").text(Post.post_need)
        ...
        const form_update = document.getElementById("form-apply-post");
        $(`#form-apply-post`).submit(function(e){
            e.preventDefault();
            let formData = new FormData(form_update)
            $.ajax({
                url: `/api/v1/seeker/${USER_ID}/posts/${Post.id}`,
                type: `POST`,
                data: formData,
                async: false,
                cache: false,
                ...
                processData: false,
                success: (response) => {
                    const SEEKERPOSTID = response.SeekerPost.id;
...
function getCookie(name) {
    ...
    for (let i = 0; i < cookies.length; i++) {
        ...
        // Mengecek apakah nama cookie cocok
